...
class SousuoParser {
  ...
  // 通用配置
  static const int _timeoutSeconds = 8; // 请求和DOM检测超时时间
  static const int _maxStreams = 8; // 最大提取媒体流数量
  // 时间常量 - 页面和DOM相关
  static const int _waitSeconds = 2; // 页面加载和表单提交后等待时间
  ...
  // 内容变化防抖时间
  static const int _contentChangeDebounceMs = 300; // 内容变化防抖时间（毫秒）

  /// 解析搜索页面并提取媒体流地址
  static Future<String> parse(String url) async {
    final completer = Completer<String>(); // 异步完成器，返回解析结果
    final List<String> foundStreams = []; // 存储提取的媒体流地址
    Timer? timeoutTimer; // 超时计时器
    WebViewController? controller; // WebView控制器
    bool contentChangedDetected = false; // 标记页面内容是否变化
    Timer? contentChangeDebounceTimer; // 内容变化防抖计时器
    bool isResourceCleaned = false; // 标记资源是否已清理
    bool isTestingStarted = false; // 标记链接测试是否开始
    bool isExtractionInProgress = false; // 标记是否正在提取操作
    // 状态对象，存储解析动态信息
    final Map<String, dynamic> searchState = {
      'searchKeyword': '', // 搜索关键词
      'activeEngine': 'primary', // 当前搜索引擎
      'searchSubmitted': false, // 搜索表单是否提交
      ...
      'lastHtmlLength': 0, // 上次HTML长度，用于增量提取
      'extractionCount': 0, // 提取计数
    };

    /// 清理WebView和相关资源
    Future<void> cleanupResources() async {
      if (isResourceCleaned) return; // 已清理，直接返回
      isResourceCleaned = true; // 标记资源清理
      LogUtil.i('开始清理资源');
      try {
        ...
        if (controller != null) {
          try {
            ...
            controller = null; // 置空控制器
          } catch (e) {
            LogUtil.e('清理WebView资源出错: $e');
          }
        }
    ...
    void startStreamTesting() {
      if (isTestingStarted) {
        LogUtil.i('已开始测试流，忽略重复请求');
        return;
      }
      isTestingStarted = true;
      LogUtil.i('开始测试 ${foundStreams.length} 个流');
      if (timeoutTimer != null && timeoutTimer!.isActive) timeoutTimer!.cancel(); // 取消超时计时器
    ...
    void handleContentChange() {
      ...
      contentChangeDebounceTimer = Timer(Duration(milliseconds: _contentChangeDebounceMs), () async {
        ...
        if (searchState['searchSubmitted'] && !completer.isCompleted && !isTestingStarted) {
          ...
          controller!.runJavaScriptReturningResult('document.documentElement.outerHTML.length').then((result) {
            searchState['lastHtmlLength'] = int.tryParse(result.toString()) ?? 0; // 更新HTML长度
          }).catchError((_) {});
          searchState['extractionCount'] += 1; // 增加提取计数
          ...
          if (afterExtractCount > beforeExtractCount) {
            ...
            if (afterExtractCount >= _maxStreams) {
              ...
              LogUtil.i('找到 ${afterExtractCount} 个链接，立即测试');
              timeoutTimer?.cancel();
              startStreamTesting();
            }
          } else if (searchState['activeEngine'] == 'primary' && afterExtractCount == 0 && !searchState['engineSwitched']) {
            ...
            function simulateHumanBehavior() {
              return new Promise((resolve) => {
                ...
                function randomScroll() {
                  ...
                  console.log("随机滚动: " + scrollAmount + "px");
                }
    ...
    try {
      ...
      await controller!.setNavigationDelegate(NavigationDelegate(
        onPageStarted: (String pageUrl) async {
          ...
          if (searchState['engineSwitched'] && _isPrimaryEngine(pageUrl) && controller != null) {
        ...
        onWebResourceError: (WebResourceError error) {
          ...
          if (searchState['activeEngine'] == 'primary' && error.url != null && error.url!.contains('tonkiang.us')) {
        ...
        onNavigationRequest: (NavigationRequest request) {
          if (searchState['engineSwitched'] && _isPrimaryEngine(request.url)) {
            LogUtil.i('阻止主引擎导航');
      ...
      await controller!.addJavaScriptChannel(
        ...
        onMessageReceived: (JavaScriptMessage message) {
          ...
          if (message.message == 'FORM_SUBMITTED') {
            ...
            resetTimeoutTimer(); // 重置超时计时器
            _injectDomChangeMonitor(controller!, 'AppChannel'); // 注入DOM变化监听器
      ...
      await controller!.loadRequest(Uri.parse(_primaryEngine)); // 加载主搜索引擎
      timeoutTimer = Timer(Duration(seconds: _timeoutSeconds), () {
        LogUtil.i('搜索超时，找到 ${foundStreams.length} 个流');
        if (!completer.isCompleted) {
          if (foundStreams.isEmpty) {
      ...
      if (!isResourceCleaned) await cleanupResources(); // 确保资源清理
    }
  }
