...
''';
  ...
  Future<DateTime?> _getNetworkTime(String url) async {
    ...
    final response = await HttpUtil().getRequest<String>(
      url,
      retryCount: 1,
      cancelToken: cancelToken, // 修改：传递 cancelToken
    );
  ...
  /// 加载时间拦截器脚本
  Future<String> _loadTimeInterceptorScript() async {
    try {
      String script = await rootBundle.loadString(_TIME_INTERCEPTOR_PATH);
      if (_cachedTimeOffset != null && _cachedTimeOffset != 0) {
        // 替换时间偏移变量
        script = script.replaceFirst('const timeOffset = 0;', 'const timeOffset = ${_cachedTimeOffset};');
      }
      return script;
    } catch (e) {
      LogUtil.e('加载时间拦截器脚本失败: $e');
      return '(function(){})();'; // 返回空函数作为备用
    }
  }
  
  /// 加载点击处理脚本
  Future<String> _loadClickHandlerScript() async {
    if (clickText == null || clickText!.isEmpty) {
      return '(function(){})();'; // 无点击操作时返回空函数
    }
    
    try {
      String script = await rootBundle.loadString(_CLICK_HANDLER_PATH);
      // 替换搜索文本和索引
      script = script.replaceFirst('const searchText = "";', 'const searchText = "$clickText";');
      script = script.replaceFirst('const targetIndex = 0;', 'const targetIndex = $clickIndex;');
      return script;
    } catch (e) {
      LogUtil.e('加载点击处理脚本失败: $e');
      return '(function(){})();'; // 返回空函数作为备用
    }
  }
  
  /// 加载M3U8检测器脚本
  Future<String> _loadM3U8DetectorScript() async {
    try {
      String script = await rootBundle.loadString(_M3U8_DETECTOR_PATH);
      // 替换文件模式
      script = script.replaceFirst('const filePattern = "m3u8";', 'const filePattern = "$_filePattern";');
      return script;
    } catch (e) {
      LogUtil.e('加载M3U8检测器脚本失败: $e');
      return '(function(){window._m3u8DetectorInitialized = true;})();'; // 返回最小初始化函数作为备用
    }
  }
...
Future<void> _initController(Completer<String> completer, String filePattern) async {
  ...
  try {
    ...
    // 加载时间拦截器脚本
    initScripts.add(await _loadTimeInterceptorScript());
    ...
    // M3U8检测器核心脚本
    initScripts.add(await _loadM3U8DetectorScript());
...
Future<bool> _executeClick() async {
  ...
  try {
    // 加载点击处理脚本
    String script = await _loadClickHandlerScript();
  ...
  void _setupPeriodicCheck() {
    ...
    _periodicCheckTimer = Timer.periodic(
      ...
      (timer) async {
        ...
        if (_m3u8Found || _isCancelled()) { // 修改：检查 cancelToken 状态
          timer.cancel();
...
/// 释放资源 - 修改：适配 nullable _controller 和 cancelToken
...
Future<void> _handleM3U8Found(String url, Completer<String> completer) async {
  ...
  /// 注入M3U8检测器
  void _injectM3U8Detector() async {
    if (_isCancelled() || !_isControllerReady() || _isDetectorInjected) { // 修改：检查 cancelToken 状态
    ...
    // 加载M3U8检测器脚本
    final detectorScript = await _loadM3U8DetectorScript();
    
    // 注入脚本并检查检测器是否正常工作
    _controller!.runJavaScript(detectorScript);
