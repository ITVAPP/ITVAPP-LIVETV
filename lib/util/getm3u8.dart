import 'dart:async';
import 'dart:convert';
import 'package:flutter/services.dart' show rootBundle; // 添加依赖以加载资产
...
/// 地址获取类
class GetM3U8 {
  static const _protocolPattern = UrlUtils._protocolPattern;
  ...
  // 用于检查协议的正则（已移除，改为使用 UrlUtils.hasValidProtocol）
  ...
  /// 无效URL关键词（已移至类级别 _invalidPatternRegex）
  ...
  /// 标记 JS 检测器是否已注入（已移除，改为通过 _controller 检查）
  // bool _isDetectorInjected = false;

  /// 规则列表
  final List<M3U8FilterRule> _filterRules;

  /// 标记页面是否已处理过加载完成事件（已移除，改为 _pageLoadedStatus 管理）
  ...
  /// 无效URL检查的正则表达式（已移至类级别 _invalidPatternRegex）
  // static final _invalidPatternRegex = RegExp(
  ...
  static List<M3U8FilterRule> _parseRules(String rulesString) {
    return rulesString.isEmpty ? [] : rulesString.split('@').where((rule) => rule.isNotEmpty).map(M3U8FilterRule.fromString).toList();
  }

  /// 解析动态关键词规则
  static Set<String> _parseKeywords(String keywordsString) {
    return keywordsString.isEmpty ? {} : keywordsString.split('@').map((keyword) => keyword.trim()).toSet();
  }

  /// 解析特殊规则字符串，返回 Map，其中键是域名，值是文件类型
  static Map<String, String> _parseSpecialRules(String rulesString) {
    return rulesString.isEmpty ? {} : Map.fromEntries(rulesString.split('@').map((rule) => MapEntry(rule.split('|')[0].trim(), rule.split('|')[1].trim())));
  }
  ...
  String _cleanUrl(String url) {
    ...
    if (UrlUtils.hasValidProtocol(cleanedUrl)) {
      return cleanedUrl;
    }
...
Future<void> _initController(Completer<String> completer, String filePattern) async {
  try {
    ...
    try {
      ...
      if (httpdata != null) {
        ...
        if (_isHtmlContent) {
          ...
          // 标记已检查,避免 WebView 重复检查（已移除 _isPageLoadProcessed）
        }
      } else {
    ...
    if (!isHashRoute && !_isHtmlContent) {
      ...
      // _isDetectorInjected = true;  // 标记为已注入，避免后续注入（已移除）
      _isControllerInitialized = true;
      // 直接调用内容检查
      final result = await _checkPageContent();
      if (result != null) {
    ...
    // 使用异步加载时间拦截器脚本
    initScripts.add(await _prepareTimeInterceptorCode());
    ...
    // 使用异步加载M3U8检测器脚本
    initScripts.add(await _prepareM3U8DetectorCode());
...
Future<void> _handleLoadError(Completer<String> completer) async {
  if (_retryCount < 2 && !_isDisposed) {
    ...
    if (!_isDisposed) {
      // 重置页面加载处理标记和点击执行标记
      // _isPageLoadProcessed = false;（已移除）
  ...
  Future<void> dispose() async {
    ...
    // 清理 URL 相关资源
    _hashFirstLoadMap.remove(Uri.parse(url).toString()); // 移除hash路由记录
    ...
    if (_isControllerInitialized && _isHtmlContent) {
      try {
        final cleanupScript = await rootBundle.loadString('assets/js/cleanup.js');
...
Future<void> _handleM3U8Found(String? url, Completer<String> completer) async {
  ...
  /// 注入M3U8检测器（已移除，移至 _setupPeriodicCheck）
