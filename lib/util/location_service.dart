...
class LocationService {
  ...
  Future<Map<String, dynamic>> getUserAllInfo(BuildContext context) async {
      ...
      if (cachedData != null) {
        ...
        if (timestamp != null) {
          ...
          if (currentTime <= (timestamp + CACHE_EXPIRY_MS)) {
            ...
            LogUtil.i('读取本地缓存, 时间戳: ${DateTime.fromMillisecondsSinceEpoch(timestamp)}');
            return _cachedUserInfo!;
          }
        }
      }
    ...
    try {
      ...
      LogUtil.i('获取成功, IP=${userInfo['location']['ip']}, 位置=$locationString, 设备=${userInfo['deviceInfo']}, User-Agent=${userInfo['userAgent']}, 屏幕=${userInfo['screenSize']}');
      return userInfo;
    } catch (e, stackTrace) {
      LogUtil.logError('获取失败, 错误: $e', e, stackTrace);
  ...
  Future<Map<String, dynamic>> _getNativeLocationInfo() async {
    ...
    try {
      ...
      LogUtil.i('IP获取成功, IP=${ipInfo['ip']}');
      
      if (!hasPermission) {
        LogUtil.i('位置权限不可用, 回退API');
      ...
      LogUtil.logError('原生位置获取失败, 错误: $e', e, stackTrace);
      return _fetchLocationInfo();
    }
  }
  ...
  Future<bool> _checkLocationPermissions() async {
    try {
      ...
      if (!serviceEnabled) {
        LogUtil.i('_checkLocationPermissions: 位置服务未启用');
        return false;
      }
      ...
      if (permission == LocationPermission.denied) {
        ...
        if (permission == LocationPermission.denied) {
          LogUtil.i('_checkLocationPermissions: 位置权限拒绝');
          return false;
        }
      }
      
      if (permission == LocationPermission.deniedForever) {
        LogUtil.i('_checkLocationPermissions: 位置权限永久拒绝');
        return false;
      }
      return true;
    } catch (e) {
      LogUtil.e('_checkLocationPermissions: 权限检查失败, 错误: $e');
      return false;
    }
  }
    ...
    try {
      ...
      if (responseData != null) {
        ...
        if (parsedData?['ret'] == 'ok' && 
          ...
          LogUtil.i('成功, IP=${parsedData!['data']['ip']}');
          return {'ip': parsedData!['data']['ip']};
        }
      }
    } catch (e) {
      LogUtil.e('IP获取失败, 错误: $e');
    }
    ...
    if (chosenResult != null) {
      ...
      try {
        ...
        if (locationResult['city'] != _unknownCity || 
          ...
          final locationString = _formatLocationString(locationResult);
          LogUtil.i('地理编码成功, 位置=$locationString');
          return locationResult;
        }
        LogUtil.i('地理编码失败, 回退API');
      } catch (e) {
        LogUtil.e('地理编码异常, 错误: $e');
      }
    }
    
    LogUtil.i('原生定位失败, 回退API');
    return _fetchLocationInfo();
  }
  ...
  Future<Map<String, dynamic>> _processLocationResult(
      ...
      String methodName) async {
    LogUtil.i('地理编码, 方法=$methodName');
    
    try {
      try {
        await setLocaleIdentifier('zh_CN');
      } catch (e) {
        LogUtil.e('设置区域失败, 错误: $e');
      }
      ...
      if (placemarks.isNotEmpty) {
        ...
        LogUtil.i('原生地理编码成功, 位置=${place.locality}, ${place.administrativeArea}, ${place.country}');
        return {
    ...
    try {
      ...
      if (nominatimResult != null) {
        LogUtil.i('Nominatim地理编码成功, 位置=${nominatimResult['city']}, ${nominatimResult['region']}, ${nominatimResult['country']}');
        return {
          'ip': ipInfo['ip'] ?? _unknownIP,
      ...
      LogUtil.e('Nominatim地理编码失败, 错误: $e');
    }
    
    LogUtil.i('地理编码失败, 使用坐标信息');
    return {
      'ip': ipInfo['ip'] ?? _unknownIP,
  ...
  Future<Map<String, dynamic>?> _tryNominatimGeocoding(double latitude, double longitude) async {
    try {
      ...
      if (responseData != null) {
        ...
        if (addr != null) {
          LogUtil.i('成功, 位置=${addr['city'] ?? addr['town'] ?? addr['village']}, ${addr['state'] ?? addr['province']}, ${addr['country']}');
          return {
        ...
        try {
          ...
          if (responseData != null) {
            ...
            if (parsedData != null) {
              ...
              if (result != null) {
                LogUtil.i('成功, 位置=${result['city']}, ${result['region']}, ${result['country']}');
                return result;
              }
            }
          }
          ...
          LogUtil.logError('API请求失败, URL=${api['url']}, 错误: $e', e, stackTrace);
        }
      }
    } finally {
  ...
  T? _getCachedValue<T>(String key) {
    if (_cachedUserInfo?.containsKey(key) == true) {
      ...
      if (value is T) {
        LogUtil.i('获取缓存, 键=$key, 值=$value');
        return value;
      }
    }
    LogUtil.i('未找到缓存, 键=$key');
    return null;
  }
