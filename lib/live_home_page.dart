...
class SeparatedStreamHandler {
  fvp.VideoPlayerController? _videoController; // 修改点：将 _videoController 的类型改为 fvp.VideoPlayerController
  fvp.VideoPlayerController? _audioController; // 修改点：将 _audioController 的类型改为 fvp.VideoPlayerController
  Timer? _syncTimer;
  ...
  // 初始化播放器
  Future<fvp.VideoPlayerController> initialize(String url, Map<String, String> headers) async {
    try {
      ...
      // 使用 `fvp.VideoPlayerController` 初始化视频控制器
      _videoController = fvp.VideoPlayerController.network(
        streams['video']!,
      ...
      if (streams['audio']!.isNotEmpty) {
        _audioController = fvp.VideoPlayerController.network(
          streams['audio']!,
          httpHeaders: combinedHeaders,
          videoPlayerOptions: VideoPlayerOptions(
            mixWithOthers: true,
...
Future<void> _playVideo() async {
    ...
    try {
        ...
        fvp.VideoPlayerController newController;
        ...
        if (isSeparatedStream) {
            ...
            // 创建普通播放器控制器
            newController = fvp.VideoPlayerController.network(
                Uri.parse(parsedUrl),
                httpHeaders: headers,
