...
double _categoryViewportHeight = 0;
double _groupViewportHeight = 0;
double _channelViewportHeight = 0;
double _epgViewportHeight = 0;
...
class _ChannelDrawerPageState extends State<ChannelDrawerPage> with WidgetsBindingObserver {
  ...
  // 修改部分：优化 _scrollToCurrentItem，确保 GroupList 和 ChannelList 焦点始终在顶部或底部对齐
  void _scrollToCurrentItem(ScrollController controller, int index, {required bool alignToBottom}) {
    if (!controller.hasClients) return;

    const itemHeight = defaultMinHeight + 12.0 + 1.0; // 55.0，固定项高度
    ...
    // 只对 GroupList 和 ChannelList 应用新逻辑
    if (controller == _scrollController || controller == _scrollChannelController) {
      if (alignToBottom) {
        // 下移：焦点项保持在底部
        if (itemBottom > viewportBottom || itemTop < viewportTop) {
          adjustedOffset = itemBottom - viewportHeight; // 底部对齐
        } else {
          adjustedOffset = currentOffset; // 未超出视窗，不滚动
        }
      } else {
        // 上移：焦点项保持在顶部
        if (itemTop < viewportTop || itemBottom > viewportBottom) {
          adjustedOffset = itemTop; // 顶部对齐
        } else {
          adjustedOffset = currentOffset; // 未超出视窗，不滚动
        }
      }
    } else {
      // 其他列表（CategoryList 和 EPGList）保持原有逻辑
      adjustedOffset = alignToBottom
  ...
  // 修改部分：优化 scrollTo，支持动态对齐，确保焦点在视窗内
  void scrollTo({
    ...
    ScrollController? scrollController;
    int maxIndex = 0;
    bool dynamicAlignToBottom = alignToBottom;
    ...
    switch (targetList) {
      ...
      case 'group':
        ...
        // 如果是当前播放频道所在分组，优先底部对齐
        if (index == _groupIndex && widget.playModel?.group == _keys[index]) {
          dynamicAlignToBottom = true;
        }
        break;
      case 'channel':
        ...
        maxIndex = _values.isNotEmpty && _groupIndex >= 0 && _groupIndex < _values.length
            ...
            : 0;
        // 如果是当前播放频道，优先底部对齐
        if (index == _channelIndex && widget.playModel?.title == _values[_groupIndex].keys.toList()[index]) {
          dynamicAlignToBottom = true;
        }
        break;
      case 'epg':
    ...
    if (scrollController != null && scrollController.hasClients) {
      const itemHeight = defaultMinHeight + 12.0 + 1.0;
      final totalHeight = (maxIndex + 1) * itemHeight;
      final viewportHeight = scrollController.position.viewportDimension;
      if (targetList == 'group' || targetList == 'channel') {
        if (totalHeight > viewportHeight && index * itemHeight > viewportHeight) {
          dynamicAlignToBottom = true; // 超出视窗时底部对齐
        } else {
          dynamicAlignToBottom = false; // 未超出时顶部对齐
        }
      }
      _scrollToCurrentItem(scrollController, index, alignToBottom: dynamicAlignToBottom);
    }
  }
  ...
  // 修改部分：优化 _adjustScrollPositions，使用动态对齐
