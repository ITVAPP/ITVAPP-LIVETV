...
class _SplashScreenState extends State<SplashScreen> {
  late Future<M3uResult> _m3uDataFuture; // 用于存储异步获取的M3U 数据结果
    ...
    try {
      ...
      LogUtil.safeExecute(() async {
        ...
        if (mounted && m3uResult.data != null) {
          ...
          // 数据获取失败时更新提示信息
          setState(() {
            _message = S.current.getm3udataerror;
          });
        }
      }, '初始化应用时发生错误');
      ...
      setState(() {
        _message = S.current.getDefaultError;
      });
    }
  }
  ...
  Future<M3uResult> _fetchData() async {
    try {
      // 更新界面提示状态
      setState(() {
        _message = S.current.getm3udata; // 显示"正在获取数据..."
      });
      ...
      // 处理获取结果
      if (result?.data != null) {
        return result!; // 成功则返回 M3uResult的数据
      } else {
        // 自动重试全部失败时显示错误
        setState(() {
          _message = S.current.getm3udataerror;
